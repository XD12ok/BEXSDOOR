...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <span class="text-lg font-bold">Daftar Produk</span>
        <a href="{{ route('products.create') }}"
           class="text-sm px-4 py-2 rounded border">
            + Tambah Produk
        </a>
    </div>
    ...
    <table class="w-full text-sm text-left shadow text-gray-700">
        <thead>
        <tr>
            <th>Gambar</th>
            <th>Product Name</th>
        ...
        @foreach ($products as $product)
            <tr class="border-t">
                ...
                <td class="space-x-2">
                    <a href="{{ route('products.edit', $product->id) }}" class="px-3 py-1 text-blue rounded hover:text-blue-600">Edit</a>
